...
static void expandkey(u32 ret[], int r, byte key[], int keylen);
static u32 subword(u32 n);

static void subbytes(byte data[16]);
static void shiftrows(byte data[16]);
static void mixcolumns(byte data[16]);

static void rsubbytes(byte data[16]);
static void rshiftrows(byte data[16]);
static void rmixcolumns(byte data[16]);
/* inverse operations */

static void addroundkey(byte data[16], u32 key[], int round);
...
/* rc[0] = 0 because it's never accessed and it's more convenient to shift all
 * the indices */
...
void aes256encrypt(byte ret[16], byte data[16], byte key[32]) {
	...
	if (ret == NULL)
		...
		memcpy(ret, data, 16);
	expandkey(expanded, 15, key, 32);

	addroundkey(ret, expanded, 0);
	for (round = 1; round < 14; ++round) {
		subbytes(ret);
		shiftrows(ret);
		mixcolumns(ret);
		addroundkey(ret, expanded, round);
	}
	subbytes(ret);
	shiftrows(ret);
	addroundkey(ret, expanded, round);
...
static void expandkey(u32 ret[], int r, byte key[], int keylen) {
	...
	for (i = 0; i < 4 * r; ++i) {
		if (i < n) {
			...
			ret[i] = ret[i - n] ^
			         subword(RTL(ret[i - 1], 8)) ^
			         (rc[i / n] << 24);
		else if (n > 6 && i % n == 4)
			ret[i] = ret[i - n] ^ subword(ret[i - 1]);
		else
			ret[i] = ret[i - n] ^ ret[i - 1];
	}
}
...
static void mixcolumns(byte data[16]) {
	...
	for (i = 0; i < 16; i += 4) {
		...
		data[i + 0] = m2[c[0]] ^ m3[c[1]] ^ c[2] ^ c[3];
		data[i + 1] = c[0] ^ m2[c[1]] ^ m3[c[2]] ^ c[3];
		data[i + 2] = c[0] ^ c[1] ^ m2[c[2]] ^ m3[c[3]];
		data[i + 3] = m3[c[0]] ^ c[1] ^ c[2] ^ m2[c[3]];
	}
}
...
static void rmixcolumns(byte data[16]) {
	...
	for (i = 0; i < 16; i += 4) {
		byte c[4];
		memcpy(c, data + i, sizeof c);
		data[i + 0] = m14[c[0]] ^ m11[c[1]] ^ m13[c[2]] ^ m9[c[3]];
		data[i + 1] = m9[c[0]] ^ m14[c[1]] ^ m11[c[2]] ^ m13[c[3]];
		data[i + 2] = m13[c[0]] ^ m9[c[1]] ^ m14[c[2]] ^ m11[c[3]];
		data[i + 3] = m11[c[0]] ^ m13[c[1]] ^ m9[c[2]] ^ m14[c[3]];
	}
}
...
static void addroundkey(byte data[16], u32 key[], int round) {
	int i;
	for (i = 0; i < 4; ++i) {
		const u32 word = key[round*4 + i];
		int j;
		for (j = 0; j < 4; ++j) {
			const int shift = (4 - j - 1) * 8;
			data[i*4 + j] ^= (word >> shift) & 0xff;
		}
	}
}
